...
public class ResourceManager {
	...
	public Drawable getDrawable(String name) {
        int id = 0;
		try {
            id = mCurrentResources.getIdentifier(appendSuffix(name), DEFTYPE_DRAWABLE, mCurrentPluginPackageName);
            if(id == 0){
                id = mCurrentResources.getIdentifier(appendSuffix(name), DEFTYPE_MIPMAP, mCurrentPluginPackageName);
            }
			return mCurrentResources.getDrawable(id);
		} catch (Exception e) {
			e.printStackTrace();
			if (mCurrentResources != mDefaultResources) {
				try {
                    id = mDefaultResources.getIdentifier(name, DEFTYPE_DRAWABLE, mDefaultPackageName);
                    if(id == 0){
                        id = mDefaultResources.getIdentifier(name, DEFTYPE_MIPMAP, mDefaultPackageName);
                    }
					return mDefaultResources.getDrawable(id);
				} catch (Exception e2) {
					e2.printStackTrace();
				}
			}
